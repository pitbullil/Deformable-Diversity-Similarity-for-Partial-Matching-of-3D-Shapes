...
It holds that the longer a distance between 2 points is, the more likely it is that a distortion had occurred on the geodesic path between them. 
Thus, instead of using the entire part for a correspondence between 2 points, we use a subset of it for the matching. 
Instead of looking for one central point as done in the original paper, we sample many points and treat each of them as a separate template for matching. 
Thus obtaining better localization. Even though we do not consider the entire part, and thus do not enforce global coherency, matches stemming from this framework are overwhelmingly correct, and this give way to simple scheme of pruning bad matches.
...
Therefore, our algorithm, which is illustraed inFigure XXX, consists of the following steps.
\begin{enumerate}
    \item 
    Pre-processing. 
\end{enumerate}
