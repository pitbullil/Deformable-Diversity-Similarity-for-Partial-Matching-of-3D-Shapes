...
We first calculate point-wise descriptor by obtaining mesh normals and calculating FPFH shape descriptors for every points on both meshes.
...
We then traverse every point $m_j \in \mathcal{M}$, and calculate the deformable diversity similarity measure between its surrounding geodesic disc $m_{j,R_T}$ to each sample template.
We determine our correspondences  each sample point $s_i\in S$ we set the point in $\hat{m}_i\mathcal{M}$ whose surrounding geodesic disc maximizes the similarity to $s_{i,R_T}$ has it's correspon 
