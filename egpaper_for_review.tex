...
\begin{enumerate}
    ...
    \nadav{proposed rewrite of the above:{\color{magenta} Then, in an iterative manner, we choose the sample point whose distance to the set is the minimal for the set of points whose distance to the set is at least $R_{Sample}$}}
...
Given $\mathcal{M}$ and $\mathcal{N}$ (Section~\ref{sec:approach}) and a vertex $v \in  \mathcal{N}$ \ayellet{yes?, not the opposite?}, the goal is to find the most similar vertex on $\mathcal{N}$.
