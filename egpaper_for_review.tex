...
\nadav{proposed text: {\color{magenta}
the smaller the compared segments are, the more likely it is for both distances and matches to remain consistent, and the less likely it is for repeating patterns which lead to lower diversity of the mapping to occur}}.

Third, multi-scale approach is beneficial, since \ayellet{in one sentence why}

\nadav{proposed text: {\color{magenta}
coarser scales obtain results which are globally consistent, but inexact , while finer scales lead to better localization, but alone might lead to mappings to a similar point which lie in a different region.}}
...
Our goal is to measure the similarity between two sets of points, the template points ${\mathcal{N}_c}^{R_T}=\{n_k\}\in\mathcal{N},d({n^l}_k,{n^l}_c)<R_T$ ,and the target points ${\mathcal{M}^{R_T}_{c'}}=\{m_k'\}\in\mathcal{N},d({m^l}_k,{m^l}_c')<R_T$, where $d$ is a given spatial distance function and ${n^l}_c,{m^l}_c'$ are given reference locations on $\mathcal{N},\mathcal{M}$ respectively. 
...
\begin{equation}
DDIS({\mathcal{N}_c}^{R_T},{\mathcal{M}^{R_T}_{c'}},\gamma)=\sum_{n_i\in {\mathcal{N}^{R_T}_{c}}}\frac{1}{1+r_i}
\end{equation}
...
%Fourth paragraph: outline of similarity computation
\nadav{outline of similarity computation: proposed text: {\color{magenta}
%preprocessing
...
We then collect a neighborhood ${\mathcal{N}_c}^{R_T}$ of a given Radius $R_T$ around a desired source point $n_c$ and compute the distance of all points which lie within this neighborhood to the source $n_c$.
...
Finally we go over all points $n\in{\mathcal{N}_c}^{R_T}$ and sum their contribution to obtain a similarity score.
}}

%paragraphs/subsections explaining the different stages
\nadav{subsections explaining the different stages, proposed text: {\color{magenta}
