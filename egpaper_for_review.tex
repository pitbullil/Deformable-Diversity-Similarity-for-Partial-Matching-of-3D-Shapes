...
However, as a post-processing step, we convert the set of sparse correspondences to a dense set using the method of \cite{litany2017fully}.
...
\begin{figure*}[htb]
	...
	\begin{tabular}[width=0.8\textwidth]{c|ccc|ccc}
	     input &  \cite{rodola2017partial} & ours-dense & ours-sparse 
	           & \cite{rodola2017partial} & ours-dense & ours-sparse  \\ \hline
	\includegraphics[scale=0.5]{figures/horse_base.png} & \includegraphics[scale=0.5]{figures/holes_horse_12_PFM.png}  & \includegraphics[scale=0.5]{figures/holes_horse_12.png} & \includegraphics[scale=0.5]{figures/holes_horse_12_sparse.png} & \includegraphics[scale=0.5]{figures/holes_horse_16_PFM.png}&
	\includegraphics[scale=0.5]{figures/holes_horse_16.png} & \includegraphics[scale=0.5]{figures/holes_horse_16_sparse.png} \\ \includegraphics[scale=0.5]{figures/dog_base.png} &
	 \includegraphics[scale=0.5]{figures/cuts_dog_8_PFM.png}  & \includegraphics[scale=0.5]{figures/cuts_dog_8.png} & \includegraphics[scale=0.5]{figures/cuts_dog_8_sparse.png} & \includegraphics[scale=0.5]{figures/holes_dog_13_PFM.png} & \includegraphics[scale=0.5]{figures/holes_dog_13.png} & 
	 \includegraphics[scale=0.5]{figures/holes_dog_13_sparse.png}\\
	\end{tabular}
	\caption{{\bf Results on examples from SHREC'16A.} 
	The left column shows the input model in a neutral pose.
	The other columns compare our results (both dense and sparse) to SOTA results of~cite{rodola2017partial}, for which the code is provided, on partial deformed models that also have many holes.
...
\begin{figure}[htb]
	...
	\end{tabular}
	\caption{{\bf Errors on examples from SHREC'16A.} 
		The error is color-coded from white (no error) to red (large error). 
		Our method evidently is less erroneous than~\cite{rodola2017partial}
	}
	\label{fig:Shrec16Qualitative-error}
\end{figure}

Similarly, Figure~\ref{fig:Shrec16TopImage} shows a couple of examples from {\em SHREC'16B}, which have topological noise.
Generally, our method outperforms that of ~\cite{rodola2017partial}.
It sometimes fails near the regions of the topological noise (i.e., when regions intersect), but the failures are fewer and scarce.

\begin{figure}[htb]
	\centering
	\begin{tabular}{ccc}
	   input & our-dense & \cite{rodola2017partial}  \\
	    	\includegraphics[scale=0.4]{figures/Top1Base.png} & \includegraphics[scale=0.4]{figures/Top1DDIS.png} & \includegraphics[scale=0.4]{figures/Top1PFM.png} \\ \includegraphics[scale=0.4]{figures/Top2Base.png} & \includegraphics[scale=0.4]{figures/Top2DDIS.png} & \includegraphics[scale=0.4]{figures/Top2PFM.png}
	\end{tabular}
	\caption{{\bf Results on Shrec'16B.} 
	Our results outperform those of~\cite{rodola2017partial}.
	Furthermore, failures are constrained to the regions of the topological noise.}
...
Specifically, let the corresponding point of $p \in \mathcal{N}$, as found by the algorithm, be $q \in \mathcal{M}$, and let the ground truth corresponding point of $p$ be $q^* \in \mathcal{M}$.
The error for $p$ is the normalized geodesic distance between  $q$ and $q^*$ on $\mathcal{M}$:
\begin{equation}
\varepsilon(p)=\frac{GeoDist_{\mathcal{M}}(q,q^*)}{\sqrt{area(\mathcal{M})}}.
\end{equation}

Figure~\ref{fig:Shrec16Cumulative} shows the cumulative curve, which indicates the percentage of NGD errors falling below a varying threshold for {\em SHREC'16A}. 
...
as provided in the benchmark site~\cite{cosmo2016shrec}. %\hyperlink{url:shrec16A}{http://www.dais.unive.it/~shrec2016/}
...
\begin{figure}[htb]
	...
	\end{tabular}
	\caption{{\bf Cumulative normalized geodesic error (NGD) curves on SHREC'16A.} 
	...
	\ayellet{Add a dashed line below the solid line in "ours"}}
	\label{fig:Shrec16Cumulative}
\end{figure}

Figure~\ref{fig:Shrec16Part} shows the mean NGD error of the correspondence between a partial model $\mathcal{N}$ and a full model $\mathcal{M}$, as a function of the ratio between their surface area $1-area(\mathcal{N})/area(\mathcal{M})$, hereafter referred to as {\em partiality}. 
It can be seen that our method is less dependent on the partiality of the model than other models, which is a benefit of our method.
...
\begin{tabular}{ccc}
	...
	\multicolumn{3}{c}{\includegraphics[width=0.4\textwidth]{figures/SHREC16Amethods.png}}
\end{tabular}
	\caption{{\bf Mean NGD as a function of model partiality.} 
	The larger the partiality, the more missing area of $\mathcal{N}$.
	Out method increases less than others and thus, which is beneficial.}
...
\end{tabular}
	\caption{{\bf Cumulative NGD on SHREC'16B.}
	Our method is competitive with~\cite{vestner2017efficient} and better than other method.
	Note that~\cite{vestner2017efficient} do not report on results for SHREC'16A due to complexity.}
...
We implemented our code in C++  and used the Point Cloud library\cite{Rusu_ICRA2011_PCL}'s implementation for the FPFH shape descriptors, for the Nearest Neighbor field computation, and for the $\chi^2$ distance between the descriptors.
...
Since most of the work is devoted to computing the similarities between points, and the points are independent on each other, the obtained speedup is almost linear.
% Since computing similarity for a given patch is totally independent from its calculation for other patches, the obtained speedup is nearly linear in the number of threads. 
...
Figure~\ref{fig:FailureCases} shows two types of typical failures, the first is due to strong topological noise and the other is due to highly-complex deformation.
In both cases this is due to two reasons: the FPFH descriptor does not capture the shape of the vertices well and the geodesic distance err due to the elasticity of some of the models.
We note that competing methods also fail on these examples.
%	(1)Surface patches such as the dog's tail that are low in distinct shapes are hard to match. 
%	(2)Topological noise due to the intersection of different model parts changes the geodesic paths of the deformed model w.r.t to the base model.
...
\end{tabular}
\caption{{\bf Limitations.} Our method might fail in cases of topological noise (top) or highly-deformed partial shapes (bottom).
\ayellet{change the bottom images}}
...
% 		\\
% 		\includegraphics[scale=0.7]{figures/cat_base.png} & \includegraphics[scale=0.4]{figures/cat_7_non_greedy.png} & \includegraphics[scale=0.4]{figures/cat_7_greedy.png} \\ 
% 	\end{tabular}
