...
\begin{enumerate}
    ...
    Specifically, $R_T=\beta\cdot \sqrt{Area(\mathcal{M})}$ and $\beta$ is a hyper parameter obtained by minimizing the geodesic error of correspondences on a training set. \ayellet{What is $R_M$? How exactly do we get $beta$?}
    
    \nadav{proposed text: In order to consider deformation ,for every such patch we also calculate the geodesic distance from the source point for every vertex on it.}

    We extract surrounding patches of the same radius around every vertex in $\mathcal{M}$ (densely, without sampling).

    %Text regarding the multiscale idea
    \nadav{proposed text for multiscale: To implement the multi-scale idea, 3 such patches are extracted for every point - at radiuses $R_{T_1}=0.6\sqrt{Area(\mathcal{M})},R_{T_2}=0.4\sqrt{Area(\mathcal{M})},R_{T_3}=0.2\sqrt{Area(\mathcal{M})}$(These values were obtained by minimizing the geodesic error on the SHREC16 Training set.)
 ...
 %   \nadav{We repeat this process for every $m\in\mathcal{M}$ to create a set of geodesic discs $\{{\mathcal{M}_{m_j}^{R_T}}\}_{j=1}^{|\mathcal{M}|}$}
    ...
    %\nadav{ we measure similarity of patches to a patch c on N and then choose the center of patch achieving the maximal similarity score as the corresponding point to the center of c. This is also the way it is defined in Lihi's paper, the rewrite is blow} 
    ...
    \nadav{proposed text:
    We now calculate a similarity score for each patch from $\mathcal{M}$ to all sample patches of the same radius on $\mathcal{N}$. }
    ...
    We calculate it, obtaining a set of $N$ similarity scores for every pair of \nadav{patches surrounding} a sample point and a vertex.
    \ayellet{what? so why did you extract patches if they are never used?}\nadav{the similarity is always calculated between the patches surrounding a point, with the point serving as a reference for geodesic distance}
    \ayellet{What is N?}\nadav{N was set to 3 - i only tested 2 and 3 scales}
    ...
    \nadav{We now use the similarity calculated between patches to obtain a mapping from our sample set of $\mathcal{N}$ to $\mathcal{M}$}.
    ...
    The corresponding point of a sample from $\mathcal{N}$ on $\mathcal{M}$ should maximize the similarity score between their corresponding patches}.
    ...
    %this would be the process if we only used a single size of geodesic disc
    \nadav{proposed text: When using a single radius size, this can be done simply by finding for each patch surrounding a sample point, the patch in $\mathcal{M}$ whose similarity to it is maximal, and setting its origin point as the correspondence.
    
    %this is the multiscale addition
    In order to implement our multi scale idea, we first use the larger patch similarity to obtain a rough estimation of the correspondence. 
    We then use this rough estimation to constrain the region on $\mathcal{M}$ in which we consider the similarity score of smaller patches to be valid, to a smaller disc surrounding the rough correspondence. This is repeated until we reach our smallest patch resolution. The final correspondence is the one that maximizes the patch similarity score, subject to lying in on the region defined by the preceding coarser scale.
    }
    
    
    \nadav{older text:
    The location of $c\in\S_\mathcal{N}$ on $\mathcal{M}$ is determined in the following way, given in Figure ~\ref{fig:multiscale}. We begin with the similarity obtained for $R_{T_1}$ and find the vertex whose associated patch similarity score $DDIS_{\mathcal{N}_c^R_{T_1}}$ is maximal. We denote it $\hat{m}_{R_{T_1}}$ on $\mathcal{M}$. Then we move sequentially from $R_{T_2}$ to $R_{T_N}$. For each $i$ we construct a geodesic disc of $R_{T_i}$ around $\hat{m}_{R_{T_{i-1}}}$. We then set $\hat{m}_{R_{T_i}}$ as the vertex on this disc which maximizes $DDIS_{\mathcal{N}^{R_{T_i}}_{c}}(\mathcal{M}_{m_j}^{R_{T_i}})$. Finally the location of $c$ is determined at the smallest radius,  $\hat{m}_{R_{T_N}}$. We repeat this process for every sample point to obtain our sparse correspondences.}
