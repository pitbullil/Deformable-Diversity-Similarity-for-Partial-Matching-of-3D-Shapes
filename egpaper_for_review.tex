...
%Problem definition
This section elaborates on Step 3 of the approach, which is the core of the algorithm.
Given pair of patches of the same scale, $Q \subset \mathcal{M}$ and $P \subset \mathcal{N}$, this section defines a similarity function $DDIS$ between them.
...
%Key idea
Recall that the key idea is to reward a point (center of the patch) whose nearest-neighbor field satisfied two properties: has high diversity and low deformation. 
\ayellet{copy two sentences: 1. why high diversity is good. 2. why low deformation is good}

Te realize these requirements, we start by formally defining diversity and  deformation and then show how to put them together into the similarity function.

We first define $Div(Q,P)$, which measures the diversity from  $Q$ to $P$,
\ayellet{paragraph: copy the definition of diversity:  one sentence of key idea of the equation that relates to the goal, and then put down the equation.}

Next, we define $Def(Q,P)$, which measures the deformation from  $Q$ to $P$.
\ayellet{one sentence of key idea of the equation that relates to the goal}
Let $p \in P$ be the center of $P$ and similarly, $q \in Q$ be the center of $Q$;
let $p_j$ be the Nearest-neighbor of  $q_i \in Q$, previously computed for all $q_i \in Q$.
\ayellet{re-write the equation correctly.}
\nadav{
...
Having defined diversity and deformation, the similarity as follows:
\begin{equation}
DDIS(P,Q)=\sum_{p_i\in \mathcal{P}}\frac{1}{1+r_i}
\label{eq:DDIS}
\end{equation}

%Mathematical definition that realizes the key idea
\nadav{
