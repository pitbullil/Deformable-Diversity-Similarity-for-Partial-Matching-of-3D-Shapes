...
\begin{figure*}[htb]
    ...
    After computing the nearest-neighbor field (Step 1), $\mathcal{N}$ is being sampled (Step 2).
    Based on the sampling, patches of  $\mathcal{N}$ are extracted for every sample point in $\mathcal{N}$ and for every vertex on $\mathcal{M}$  (Step 3). 
    We calculate the similarity from every vertex of M to every sample of N. Then for every sample of N we set the vertex on M which achieves the maximal score as its match.
...
\begin{enumerate}
    ...
    ======
    First, for each sample point in the set, we build a "forbidden" geodesic region of a pre-defined radius around it \nadav{of 5\% of the surface characteristic length $R_\mathcal{M}=\sqrt{Area(\mathcal{M})}$. }. \ayellet{how do we select this radius?}
    The next point cannot fall in any of these regions.
    Then, we choose the next sample as the vertex whose geodesic distance to any sample point in the set is minimal. \nadav{This process is repeated until the entire surface is marked as a forbidden region. The resulting set of samples is denoted as  $\S_{\mathcal{N}}\in\mathcal{N}$ }.
    ...
    \ayellet{how do we select $R_T$?}\nadav{ $c\in\S_{\mathcal{N}}$}, defined to be the geodesic disc of radius $R_T$ around it, \nadav{ denoted ${\mathcal{N}^{R_T}_{c}}$, where $R_T=\beta\cdot R_\mathcal{M}$ and $\beta$ is a hyper parameter tuned on a training set.}.
    Note that these sub-surfaces may overlap.
    ...
    \ayellet{is that correct or the opposite?}\nadav{comment: opposite - we measure similarity of patches to a patch c on N and then choose the center of patch achieving the maximal similarity score as the corresponding point to the center of c. This is also the way it is defined in Lihi's paper, the rewrite is blow} 
    
    \nadav{proposed rewrite of the above paragraph,Computing the similarity between all pairs of patches:
    ...
    The idea is to reward a NN field with high diversity and low deformation. \nadav{We denote the similarity function realizing this as $DDIS_{\mathcal{N}^{R_T}_{c}}(\mathcal{M}_{m}^{R_T})$.} %Unlike\cite{talmi2017template}, we have found it beneficial not to penalize points which share the same nearest neighbor, but rather only use the one which has the lowest deformation.
    ...
    Steps 3,4 are performed for $N$ different values of $R_T$, $R_{T_1}>...>R_{T_N}$, and thus we obtain for a pair of sample point and vertex, a set N different similarity scores $\{DDIS_{\mathcal{N}^{R_{T_i}}_{c}}(\mathcal{M}_{m_j}^{R_{T_i}})\}_{i=1}^N$.}
...
\begin{figure}[htb]
	...
	\caption{Illustration of the Nearest Neighbor Field between different shapes. 
		...
		You can notice that on identical pieces, and even on deformed matching pieces there are multiple diverse matches, most of which are colored in blue to indicated very similar distances from the source point, whereas on different pieces most lines map to very few points and a lot of yellow lines (high deformation) exist}% A- identical,B - deformed,C - shifted,D - non related
		\label{fig:NNF}
\end{figure}
