...
\nadav{From lihi: Our key idea is that the similarity between the target and the template is captured by two properties of the implied NN field ... when the target and template correspond, most target patches have a unique NN match in the template. This implies that the NN field is highly diverse, pointing to many different patches in the template ... Second, we note that arbitrary matches typically imply a large deformation ... \cite{talmi2017template}p.1}.

\ayellet{Similarity should be based both on the diversity... and the consistency of the distances between the points.}

\nadav{proposed text: {\color{magenta}
Similarity between a template and a target should be based both on the diversity of the matches from the template to the target, as most matches should be unique leading to a highly diverse mapping, and the consistency of distances between pairs of matches.}}
...
\nadav{proposed text: {\color{magenta}
the smaller the compared segments are, the more likely it is for both distances and matches to remain consistent, and the less likely it is for repeating patterns which lead to lower diversity of the mapping}}.

Third, multi-scale approach is beneficial, since \ayellet{in one sentence why}

\nadav{proposed text: {\color{magenta}
coarser scales obtain results which are globally consistent, but inexact , while finer scales lead to better localization, but alone might lead to mappings to a similar point which lies in a different region.}}

Therefore, our algorithm, which is illustrated in \ayellet{Figure XXX}, consists of the following steps.
\begin{enumerate}
    \item 
    \textbf{Pre-processing}.
    In this step, shapes descriptors are calculated for both meshes and an approximate nearest neighbor field is computed for the shape descriptors.
    Many descriptors have been proposed in the literature.
    We are using the \ayellet{ FPFH~\cite{} due to its benefits....}
    
    \nadav{proposed text: {\color{magenta}
    FPFH due to its benefits of being robust to small deformations and partiality of data, while sensitive to symmetrical flips. }}
    \item
    ...
    \ayellet{Is this the segmentation??} \nadav{Yes. I do not call it segmentation because there isn't necessarily semantic significance. overlapping patches is more accurate...}
    \item
    ...
    \nadav{proposed text:{{\color{magenta}
    We use a modified version of the Deformable Diversity Similarity\cite{talmi2017template}, which rewards a NN field which is both high in diversity and low in deformation. Unlike\cite{talmi2017template}, we have found it beneficial not to penalize points which share the same nearest neighbor, but rather only use the one which has the lowest deformation.}}}
    The point that maximizes the similarity function is considered the corresponding point.
    \ayellet{we are missing some "meat" here} 
    \nadav{added above}
    ...
    % \end{itemize}
    \item
    \ayellet{Is there a stage that takes the point correspondences and creates surface correspondences?}\nadav{It is currently defined as optional - and it is the use of these sparse correspondences as an input to the algorithm of \cite{litany2017fully} to produce a dense set of correspondences- it is not a part of our core algorithm and I am not sure if it should be mentioned here your opinion?}
\end{enumerate}
The latter two steps are performed in a multi-scale approach. 
\ayellet{missing a couple of sentences here} 
\nadav{proposed text:{\color{magenta}
We define a set of $N$ scales characterized by their geodesic radii $R_{T_1}>...>R_{T_N}$ and calculate the similarity scores for geodsic discs at each scale. We then move from a coarse to fine scale by iteratively finding the maxima of similarity at scale $R_{T_k}$, and use to define a feasible set of point on which the maxima at scale $R_{T{k+1}}$ by constraining them to lie in a geodesic disc of radius $R_{T_k}$ around it. We repeat this process and determine the final correspondence at scale $N$}} 
...
%First paragraph: problem definition
\nadav{proposed text:{\color{magenta}
Given a pair of surfaces with a source vertex in 3 dimensions, we would like to have a way to measure the similarity between them, which will be as invariant as possible to both complex, possibly non rigid transformation, and to the challenges induced by partiality of data and topological noise.}}
...
First, the way in which 3D data is organized forces a comparison between shapes with an asymmetrical size of data, which introduces different biases, such as penalizing pieces with repeating patterns, which occur more frequently when the search window contains more data points than the template, a problem which is escalated even further by partiality.
...
%Third paragraph: the mathematical definition that realizes the key idea
\nadav{proposed text: {\color{magenta} 
Let points $n_i,m_j$ represent surface regions of the template and target respectively, with $n^l_i,m^l_j$ denoting their location, and $n^S_i,m^S_j$ denoting their shape, described by some given vector.
Our goal is to measure the similarity between two sets of points, the template points $\mathcal{N}_{c,R_T}=\{n_k\}\in\mathcal{N},d(n^l_k,n^l_c)<R_T$ ,and the target points $\mathcal{M}_{c',R_T}=\{m_k'\}\in\mathcal{N},d(m^l_k,m^l_c')<R_T$, where $d$ is a given spatial distance function and $n^l_c,m^l_c'$ are given reference locations on $\mathcal{N},\mathcal{M}$ respectively. 
We require finding the NN in $\mathcal{N}$ for every point $m\in\mathcal{M} s.t. NN(m,\mathcal{N})=argmin_{n\in\mathcal{N}}d(n^S,m^S)$ for some given distance function defined on their shape function space. The property of diversity can be measured intuitively by counting the number of unique NNs. 
Thus, same as in \cite{talmi2017template} we define Diversity as:
\begin{equation}
\underset{\mathcal{M}_{c',R_T}\rightarrow\mathcal{N}_{c,R_T}}{DIS}=|\{n_i\in\mathcal{N}_{c,R_T}:\exists m_j\in\mathcal{M}_{c',R_T},NN(m_j,\mathcal{N})=n_i\}|
\end{equation}
where $|\{.\}|$ denotes group size.

In order to take into account our second desired property of pairwise distance consistency, for each point $n\in\mathcal{N}_{c,R_T}$ we search for a point $m\in\mathcal{M}_{c',R_T}$ such that $NN(m,\mathcal{N})=n$ and whose distance to the source $d(m^l,m^l_c')$ is the most similar to $d(n^l,n^l_c)$. 
We define the deformation induced by $n_i$ as 
\begin{equation}
r_i = min(|d(m_j,c')-d(n_i,c)|),\\
\end{equation}
where $m_j\in \mathcal{M}_{c,R_T}, NN^S(m_j,\mathcal{N})=n_i$. If $n_i$ has no point who points to it as its nearest neighbor in $\mathcal{M}_{c,R_T}$ then $r_i=\infty$.
...
\begin{equation}
DDIS(\mathcal{N}_{c,R_T},\mathcal{M}_{c',R_T},\gamma)=\sum_{n_i'\in \mathcal{N}_{c,R_T}}\frac{1}{1+r_i'}
\end{equation}
...
We then collect a neighborhood $\mathcal{N}_{c,R_T}$ of a given Radius $R_T$ around a desired source point $n_c$ and compute the distance of all points which lie within this neighborhood to the source $n_c$.
%similarity calculation
We repeat the same process for a candidate point $m_c\in\mathcal{M}$ to create $\mathcal{M}_{c',R_T}$.
We traverse all the points in $\mathcal{M}_{c',R_T}$ and check for each one if it's distance to its center is the most similar to the distance of its nearest neighbor in $\mathcal{N}_{c,R_T}$ and update the contribution of $n_i$ to the similarity score accordingly.
Finally we go over all points $n\in\mathcal{N}_{c,R_T}$ and sum their contribution to obtain a similarity score.
}}
