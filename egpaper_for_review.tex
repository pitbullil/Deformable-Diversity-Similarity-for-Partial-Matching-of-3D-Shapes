...
\nadav{The SHREC'16 benchmarks accepts results of 2 distinct types. 
...
\ayellet{paragraph: explain that you compute only sparse and then use... to convert it into dense.}
 \nadav{ Our method produces sparse correspondences, but as our evaluation shows, these can be converted to dense ones with minimal loss of quality.
 This is achieved by feeding them as localized delta functions to the method of \cite{litany2017fully} instead of standard shape descriptors.}
...
\nadav{Figure~\ref{fig:Shrec16Part} Shows The mean geodesic error of a model as a function of it's partiality w.r.t. the neutral model. It can be seen that our method is pretty indifferent to the partiality of the model.}
\begin{figure}[htb]
...
\nadav{Our code which produces the sparse correspondences is implemented entirely in C++ . For geodesics we have used Fast Marching Geodesics\cite{kimmel1996fast}.
